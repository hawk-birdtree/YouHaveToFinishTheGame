...
void UpdateProjectile(float deltaTime) {
    static float globalCooldown = 0.0f;
    globalCooldown -= deltaTime;

    if(globalCooldown <= 0.0f){
        // Update projectiles every few seconds
        globalCooldown = 3.0f;  // Adjust the cooldown as needed
    }
    ...
    for(size_t i = 0; i < MAX_PROJECTILES; i++) {
        if(projectiles[i].active) {
            ...
            if (projectiles[i].cooldown > 0.0f) {
                ...
                continue;  // Skip movement if cooldown is active
            }
            ...
            if(projectiles[i].delay > 0.0f) {
                ...
                // Calculate distance travelled towards the target
                float distanceTravelled = Vector2Distance(projectiles[i].startPos, projectiles[i].position);

                // If we reach the end position, reset the projectile back to start and apply cooldown
                if (distanceTravelled >= projectiles[i].maxDistance) {
                    // Apply cooldown (add time before it can reset and move again)
                    projectiles[i].cooldown = 1.0f;  // 1 second delay after reset
                    projectiles[i].position = projectiles[i].startPos; // Reset to the start position
                    projectiles[i].delay = 1.0f; // Reset delay
                }
            }
        }
    }
...
void InitHorizontalMonsters(void) {
    ...
    for (size_t i = 0; i < MAX_HORIZONTAL_MONSTERS && i < pathIndexHorizontal; i++) {
        ...
        monsterHorizontal[i].movementDistance = MONSTER_MOVE_DISTANCE[GetRandomValue(0,4)];
    }
}
...
void InitVerticalMonsters(void) {
    ...
    for (size_t i = 0; i < MAX_VERTICAL_MONSTERS && i < pathIndexVertical; i++) {
        ...
        monsterVertical[i].movementDistance = MONSTER_MOVE_DISTANCE[GetRandomValue(0,4)];
    }
}
    ...
    if (frameDelayCounter >= frameDelay) {
        for(size_t i = 0; i < pathIndexHorizontal; i++) {
            ...
            if (monsterHorizontal[i].frame.y >= TILE_SIZE * 2) { //monsterHorizontal[0].texture.height) {
                monsterHorizontal[i].frame.y = 0; // Loop back to the first frame
            }
        }
    ...
    for (size_t i = 0; i < pathIndexHorizontal; i++) {
        if (monsterHorizontal[i].direction == 1) {
            ...
            if (monsterHorizontal[i].position.x <= (monsterHorizontalPatrolPath[i].x - monsterHorizontal[i].movementDistance)) {
                ...
                monsterHorizontal[i].position.x = (monsterHorizontalPatrolPath[i].x - monsterHorizontal[i].movementDistance);
                monsterHorizontal[i].frame.width = -monsterHorizontal[i].frame.width; // Flip frame
            }
        } else {
            monsterHorizontal[i].position.x += monsterHorizontal[i].speed;
            if (monsterHorizontal[i].position.x >= (monsterHorizontalPatrolPath[i].x + monsterHorizontal[i].movementDistance)) {
    ...
    for (size_t i = 0; i < pathIndexVertical; i++) {
        if (monsterVertical[i].direction == 1) {
            ...
            if (monsterVertical[i].position.y <= (monsterVerticalPatrolPath[i].y - monsterVertical[i].movementDistance)) {
                ...
                monsterVertical[i].position.y = (monsterVerticalPatrolPath[i].y - monsterVertical[i].movementDistance);
                monsterVertical[i].frame.height = -monsterVertical[i].frame.height; // Flip frame
            }
        } else {
            monsterVertical[i].position.y += monsterVertical[i].speed;
            if (monsterVertical[i].position.y >= (monsterVerticalPatrolPath[i].y + monsterVertical[i].movementDistance)) {
